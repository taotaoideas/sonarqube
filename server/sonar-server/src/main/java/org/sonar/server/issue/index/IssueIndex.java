...
public class IssueIndex extends BaseIndex {
  ...
  public Collection<String> searchForTags(IssueQuery query, @Nullable String tagMatch, int maxNumberOfTags) {
    Terms result = executeTagSearch(query, tagMatch, Terms.Order.term(true), maxNumberOfTags);

    return Collections2.transform(result.getBuckets(), new Function<Terms.Bucket, String>() {
      @Override
  ...
  public Map<String, Long> tagFacets(IssueQuery query, int maxNumberOfTags) {
    Terms result = executeTagSearch(query, null, Terms.Order.count(false), maxNumberOfTags);
  ...
  private Terms executeTagSearch(IssueQuery query, @Nullable String tagMatch, Terms.Order tagOrder, int maxNumberOfTags) {
    SearchRequestBuilder requestBuilder = getClient()
      .prepareSearch(IssueIndexDefinition.INDEX)
      .setTypes(IssueIndexDefinition.TYPE_ISSUE);
  ...
  public List<FacetBucket> searchForAssignees(IssueQuery query) {
    // TODO do not return hits
    ...
    List<FacetBucket> buckets = newArrayList();
    for (Terms.Bucket value : aggregation.getBuckets()) {
      buckets.add(new FacetBucket(value.getKey(), value.getDocCount()));
    }
    buckets.add(new FacetBucket("_notAssigned_", ((InternalMissing) response.getAggregations().get("notAssigned")).getDocCount()));
    return buckets;
  }
