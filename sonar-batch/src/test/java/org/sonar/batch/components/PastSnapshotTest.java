...
public class PastSnapshotTest {
  ...
  public void test_some_setters_and_getters() {
    Snapshot snapshot = new Snapshot().setQualifier(Qualifiers.FILE).setCreatedAt(new Date().getTime());
    snapshot.setId(10);
  ...
  public void testToStringForVersion() {
    PastSnapshot pastSnapshot = new PastSnapshot(CoreProperties.TIMEMACHINE_MODE_VERSION, new Date(), new Snapshot().setCreatedAt(new Date().getTime())).setModeParameter("2.3");
  ...
  public void testToStringForNumberOfDaysWithSnapshot() {
    PastSnapshot pastSnapshot = new PastSnapshot(CoreProperties.TIMEMACHINE_MODE_DAYS, new Date(), new Snapshot().setCreatedAt(new Date().getTime())).setModeParameter("30");
    assertThat(pastSnapshot.toString()).startsWith("Compare over 30 days (");
  }
  ...
  public void testToStringForDateWithSnapshot() {
    PastSnapshot pastSnapshot = new PastSnapshot(CoreProperties.TIMEMACHINE_MODE_DATE, new Date(), new Snapshot().setCreatedAt(new Date().getTime()));
    assertThat(pastSnapshot.toString()).startsWith("Compare to date ");
  }

  @Test
  public void testToStringForPreviousAnalysis() {
    PastSnapshot pastSnapshot = new PastSnapshot(CoreProperties.TIMEMACHINE_MODE_PREVIOUS_ANALYSIS, new Date(), new Snapshot().setCreatedAt(new Date().getTime()));
