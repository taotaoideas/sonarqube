...
public class Periods implements BatchComponent, ServerComponent {
  ...
  public String label(Snapshot snapshot, int periodIndex) {
    return label(snapshot.getPeriodMode(periodIndex), snapshot.getPeriodModeParameter(periodIndex), timeToDate(snapshot.getPeriodDate(periodIndex)));
  }

  @CheckForNull
  public String abbreviation(Snapshot snapshot, int periodIndex) {
    return abbreviation(snapshot.getPeriodMode(periodIndex), snapshot.getPeriodModeParameter(periodIndex), timeToDate(snapshot.getPeriodDate(periodIndex)));
  }
